...
export class Color {
  ...
  static parse(str: string): Color {
    if (typeof str == 'undefined') return Color.makeRGB(0, 0, 0); // エラー時は000にfallbackする
    if ((str = str.trim().toLowerCase()).length == 0) return Color.makeRGB(0, 0, 0); // エラー時は000にfallbackする
    if (str.startsWith('#')) return Color.makeHEX(str.substring(1, str.length));
    if (str.startsWith('rgb')) {
      const parts = str.match(/^rgba?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,?\s*(\d*(?:\.\d+\s*)?)\)$/);
      if (!parts) {
  ...
  private static Validator = {
    ...
    checkHEX: (value: string) => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (!((c >= 48 && c <= 57) || (c >= 97 && c <= 102))) throw new Error(`Hexa color: out of range for "${value}" at position ${i}.`);
      }
    },
  };
