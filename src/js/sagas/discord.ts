...
export const saveToken = async (config: RootState['content']['config']['discord']['config']): Promise<boolean> => {
 ...
 * @param code
 */
const getAccessToken = async (config: RootState['content']['config']['discord']['config'], code: string): Promise<OAuthToken | null> => {
  try {
...
export function* loginCheck() {
  try {
    ...
    if (!token) {
      yield call(saveToken, state.content.config.discord.config);
